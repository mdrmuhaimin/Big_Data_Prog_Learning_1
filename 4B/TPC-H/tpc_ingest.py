...
def read_table(keyspace, sc, input_dir, tbl, types):
    ...
    cdata = sc.textFile(infile).repartition(100).map(lambda line: to_cassandra_data(line, types)).setName(tbl)
    cdata.saveToCassandra(keyspace, tbl, consistency_level=ConsistencyLevel.ONE)
